...
/*
   ...
   #define STBTT_free(x,u)    free(x)
        ...
        static void STBTT_free<T>(FakePtr<T> p)
        {
        } // no-op
...
////      
        ...
        static int STBTT_POINT_SIZE(int x)
        {
            return -x;
        }
        ...
        public struct stbtt_pack_range
        {
            ...
            public byte h_oversample, v_oversample;
        }
...
// @TODO: don't expose this structure
        ...
        static int stbtt__buf_get(stbtt__buf b, int n)
        {
            int v = 0;
            int i;
        ...
        static stbtt__buf stbtt__new_buf(FakePtr<byte> p, uint size)
        {
            stbtt__buf r;
        ...
        static int stbtt__buf_get16(stbtt__buf b)
        {
            return (int)stbtt__buf_get(b, 2);
        }
        
        static int stbtt__buf_get32(stbtt__buf b)
        {
            return (int)stbtt__buf_get(b, 4);
        }

        static stbtt__buf stbtt__buf_range(stbtt__buf b, int o, int s)
        {
            stbtt__buf r = stbtt__new_buf(new FakePtr<byte>(), 0);
        ...
        static void stbtt__dict_get_ints(stbtt__buf b, int key, int outcount, ref FakePtr<int> outval)
        {
            int i;
            stbtt__buf operands = stbtt__dict_get(b, key);
            for (i = 0; i < outcount && operands.cursor < operands.size; i++)
                outval[i] = stbtt__cff_int(operands);
        }
        ...
        {
            //Debug.WriteLine($"Checking ({p[0]}, {p[1]}, {p[2]}, {p[3]}) against ({c0}, {c1}, {c2}, {c3})");
        ...
        public static int stbtt_GetNumberOfFonts(FakePtr<byte> font_collection)
        {
        ...
        public static int stbtt_InitFont(ref stbtt_fontinfo info, FakePtr<byte> data2, int fontstart)
        {
            ...
            info.cff = stbtt__new_buf(new FakePtr<byte>(), 0);
        ...
        public static int stbtt_GetCodepointBox(ref stbtt_fontinfo info, uint codepoint, out int x0, out int y0,
            out int x1, out int y1)
        {
            return stbtt_GetGlyphBox(ref info, stbtt_FindGlyphIndex(ref info, codepoint), out x0, out y0, out x1,
                out y1);
        }
            ...
            int sx, int sy, int scx, int scy, int cx, int cy)
        {
            ...
            {
                if (was_off != 0)
                    stbtt__setvertex(vertices + num_vertices++, GlyphVertexType.Curve, (cx + scx) >> 1, (cy + scy) >> 1,
                        cx, cy);
        ...
        public static int stbtt_GetGlyphShape(ref stbtt_fontinfo info, uint glyph_index,
            out FakePtr<GlyphVertex> pvertices)
        {
                    ...
                    else
                    {
                        if (0 == (flags & 1))
                        {
                            ...
                            if (was_off != 0
                                ...
                                stbtt__setvertex(vertices + num_vertices++, GlyphVertexType.Curve, (cx + x) >> 1,
                                    (cy + y) >> 1, cx, cy);
                            cx = x;
                            cy = y;
                            was_off = 1;
        ...
        public static int stbtt_GetCodepointKernAdvance(ref stbtt_fontinfo info, uint ch1, uint ch2)
        {
            if (0 == info.kern) // if no kerning table, don't waste time looking up both codepoint->glyphs
                return 0;
            return stbtt_GetGlyphKernAdvance(ref info, stbtt_FindGlyphIndex(ref info, ch1),
                stbtt_FindGlyphIndex(ref info, ch2));
        }

        public static void stbtt_GetCodepointHMetrics(ref stbtt_fontinfo info, uint codepoint,
            out int advanceWidth, out int leftSideBearing)
        {
            stbtt_GetGlyphHMetrics(ref info, stbtt_FindGlyphIndex(ref info, codepoint), out advanceWidth,
                out leftSideBearing);
        }
...
//
        ...
        public static void stbtt_GetGlyphBitmapBox(ref stbtt_fontinfo font, uint glyph,
            ...
            out int ix0, out int iy0, out int ix1, out int iy1)
        {
            stbtt_GetGlyphBitmapBoxSubpixel(ref font, glyph, scale_x, scale_y, 0, 0, out ix0, out iy0, out ix1,
                out iy1);
        }
        ...
        {
            stbtt_GetCodepointBitmapBoxSubpixel(ref font, codepoint, scale_x, scale_y, 0, 0, out ix0, out iy0, out ix1,
                out iy1);
        }
...
// are wrong, or if the user supplies a too-small bitmap
                        ...
                        if (i < len && j >= 0)
                        {
                            ...
                            else
                            {
                                if (i >= 0) // add antialiasing for x0
                                    scanline[i] =
                                        (byte) (scanline[i] +
                                                (byte) (((FIX - (x0 & FIXMASK)) * max_weight) >> FIXSHIFT));
                                else
                                    i = -1; // clip

                                if (j < len) // add antialiasing for x1
                                    scanline[j] =
                                        (byte) (scanline[j] + (byte) (((x1 & FIXMASK) * max_weight) >> FIXSHIFT));
                                else
                                    j = len; // clip
        ...
        static void stbtt__rasterize_sorted_edges(ref stbtt__bitmap result, FakePtr<stbtt__edge> e, int n,
            int vsubsample, int off_x, int off_y)
        {
            stbtt__active_edge
                activeIsNext =
                    new stbtt__active_edge(); // jfb: If only I could do pointers to stack...*sigh* Oh C#, why so arbitrarily limited...
            int y, j = 0;
...
// tesselate until threshhold p is happy... @TODO warped to compensate for non-linear stretching
        static int stbtt__tesselate_curve(FakePtr<ContourPoint> points, ref int num_points, float x0, float y0,
...
// returns number of contours
            ...
            int x_off, int y_off, int invert)
        {
            ...
            FakePtr<ContourPoint> windings = stbtt_FlattenCurves(vertices, num_verts, flatness_in_pixels / scale,
                out winding_lengths, out winding_count);
            if (!windings.IsNull)
            {
                stbtt__rasterize(ref result, windings, winding_lengths, winding_count, scale_x, scale_y, shift_x,
                    shift_y, x_off, y_off, invert);
            ...
            uint glyph, out int width, out int height, out int xoff, out int yoff)
        {
            ...
            if (gbm.w > 0 && gbm.h > 0)
            {
                ...
                if (!gbm.pixels.IsNull)
                {
                    ...
                    stbtt_Rasterize(ref gbm, 0.35f, vertices, num_verts, scale_x, scale_y, shift_x, shift_y, ix0, iy0,
                        1);
                }
            }
            STBTT_free(vertices);
...
// This is SUPER-SHITTY packing to keep source code small
            ...
            FakePtr<BakedChar> chardata)
        {
            ...
            for (i = 0; i < num_chars; ++i)
            {
                ...
                if (g == null)
                {
                    chardata[i] = new BakedChar() {X0 = (ushort) x, X1 = (ushort) x, Y0 = (ushort) y, Y1 = (ushort) y};
                    continue;
                }
